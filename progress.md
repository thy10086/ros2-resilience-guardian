...
- 2026-09-24: Resumed the approved ROS 2 closed-loop AMR simulation task. The existing `amr_simulator.py` model and dashboard simulation API were inspected; the new dashboard test initially failed because its HTTP fixture was missing, then passed after adding a bounded fixture (`4 passed` with simulator tests).
- 2026-09-24: Added the fifth “工程仿真” workspace. It sends only bounded `start`/`stop`/`reset`/attack commands to `/api/simulation/control`, renders phase, pose, requested/actual speed, speed limit, pallet/grip state, attack mode and mitigation, and states the Jev advisory boundary. Frontend contract red test failed before implementation and passed after implementation; `node --check` passed.
- 2026-09-24: Added `amr_simulator` to `guardian.launch.py` and documented the digital-twin data flow, topics and experiment procedure in `README.md`, `docs/warehouse_amr_case.md` and `docs/amr_closed_loop_simulation.md`.
- 2026-09-24: ROS 2 Jazzy build completed for `guardian_interfaces` and `guardian_core`. In isolated `ROS_DOMAIN_ID=42`, the real `guardian_node` and `amr_simulator` exchanged an `amr_simulator` `UNSAFE_COMMAND`; after Guardian feedback the simulator published zero-valued `/cmd_vel`, proving the safety feedback path affects simulated actuation. Full regression, local service restart and browser verification are the remaining checkpoint.
- 2026-09-24: Added `deploy/guardian-simulator.service` and enabled the matching WSL systemd unit so the 8088 deployment starts the simulator together with Guardian and dashboard. All three services are active.
- 2026-09-24: Final regression: WSL2 `python3 -m pytest -q tests` reported `211 passed`; innovation, Jev efficiency/session, patent and warehouse AMR experiments all returned `passed: true`; ROS 2 Jazzy build finished for both packages; Windows frontend syntax and diff checks passed. HTTP smoke verified `/api/health=200`, login/state access, the “工程仿真” and attack controls in served HTML, and the deployed `start → speed_abuse` path produced `CONTAINING`, `ISOLATE_COMPONENT`, requested `0.62 m/s`, actual `0.00 m/s`, speed limit `0.15 m/s`.
- 2026-09-24: A post-restart reset smoke exposed stale `attack_mode` in the dashboard cache because JSON `null` was discarded; the regression failed, `DashboardState.update_simulation()` was corrected to clear nullable attack mode, and the focused simulator/dashboard/state tests passed (`7 passed`). The final UI wording documents that Guardian safety state returns from containment only after the configured event TTL; no unauthenticated reset bypass was added.
- 2026-09-24: Final service restart and live HTTP loop were repeated after the cache fix: fresh Guardian state accepted `speed_abuse` and produced `CONTAINING`/`ISOLATE_COMPONENT` with requested `0.62 m/s`, actual `0.00 m/s`, limit `0.15 m/s`; authenticated reset then returned the digital twin to `IDLE`, cleared `attack_mode=null`, and Guardian returned `NORMAL` after the 5-second TTL.
- 2026-09-24: Refined the desktop dashboard presentation for application-facing use. The fifth workspace is now labelled “安全测试分析”; visible copy uses security testing, execution paths, validation metrics and system boundaries, with proposal-oriented wording removed from the frontend. Existing `/api/research/suite`, hash routing and internal DOM IDs remain compatible. The frontend contract test passed after the red/green cycle; WSL2 dashboard/experiment tests passed with `19 passed`, Windows `node --check` passed, and no secrets or environment variables were changed.
- 2026-09-24: Started the industrial ROS 2 code safety inspection extension. The planned boundary is source-text-only analysis for a conveyor/robot-arm profile; untrusted code will not be executed or imported. The first implementation stage is the backend contract and deterministic findings model, with Guardian action mapping kept advisory and separate from runtime control.
- 2026-09-24: Completed the industrial ROS 2 code safety inspection extension. Added `dashboard_code_security.py` with bounded AST/text rules, authenticated `/api/code-security/samples` and `/api/code-security/inspect`, a six-page dashboard workspace with file import and findings mapping, checked-in safe/unsafe conveyor-arm fixtures, and `docs/industrial_code_security.md`. Red/green contract tests, credential evidence redaction, WSL2 full regression (`220 passed`), ROS 2 Jazzy build, frontend syntax and diff checks passed; no uploaded code is executed or sent to a provider.
- 2026-09-25: Prepared the complete local `main` history for publication. Confirmed `origin` targets the intended GitHub repository without embedded credentials, `.env` remains tracked, full WSL2 regression stayed at `220 passed`, and the ROS 2 two-package build plus frontend checks passed. Added this publication entry to `HANDOFF.md`, `progress.md` and `task_plan.md`; push to `origin/main` is the final step for this turn.
