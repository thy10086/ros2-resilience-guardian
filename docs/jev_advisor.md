...
The advisor and efficient judge re-sample their clocks while holding the
cache/reservation lock. A request paused after cache lookup therefore cannot
use an older timestamp to revive an expired advisor or efficient-layer cache,
or reset a newer fixed budget window; a legitimate cache miss or budget reset
occurs only when a later request reaches the corresponding locked boundary.
